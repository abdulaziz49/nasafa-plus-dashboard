...
import type { UserActionTypes } from "../../types/user_system/user_action_type.ts"; // For useReducer's dispatch type
import type { User } from "../../../../../models/user_system/user_models.ts";
...
export const addUser = async (
    ...
    try {
        ...
        dispatch({ type: "ADD_USER_SUCCESS", payload: response.data }); // Backend should return the created role with ID
    } catch (error: any) {
        const errorMessage =
...
export const editUser = async (
    ...
    try {
        ...
        const errorMessage =
            ...
            error.message ||
            "Failed to edit role";
        dispatch({ type: "EDIT_USER_FAILURE", payload: errorMessage });
...
export const searchUser = (
    ...
    if (!stillSearch) dispatch({ type: "SEARCH_USERS_REQUEST" });
    // try {
    ...
    if (searchTerm.length > 0) {
        ...
        dispatch({ type: "SEARCH_USERS_SUCCESS", payload: searchTerm });
    } else {
        // filteredUSER = []
