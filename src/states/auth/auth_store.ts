...
import {devtools, persist} from 'zustand/middleware'; // Optional: for DevTools and persistence
...
export const useAuthStore = create<AuthState>()(
    ...
    devtools(
        persist(
        (set, get) => ({ // `set` to update state, `get` to read current state
            // Initial State
            user: emptyUser,
                ...
                return (storedState, error) => {
                    if (error) {
                        ...
                        if (storedState.token) {
                            ...
                            get().fetchUser(); // Use `get()` to access current store actions
                        } else {
