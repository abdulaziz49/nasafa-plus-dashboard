...
const AppAxios = axios.create({
	// baseURL: `http://${location.hostname}${import.meta.env.VITE_API_URL}`,
		...
		if (error.code === 'ERR_NETWORK') {
			...
			switch (status) {
				...
				default:
					errorMessage =
						data?.message ||
						`An error occurred with status ${status}.`;
					break;
			}
			toast.error(errorMessage);
			...
			errorMessage =
				'No response from server. It might be down or unreachable.';
